...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.litecoin.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
  - `xpub` encoded extended public key or `xprv` encoded extended private key (as defined in [BIP 32](https://github.com/litecoin/bips/blob/master/bip-0032.mediawiki)).
...
- P2SH addresses (base58, of the form `3...` for mainnet or `2...` for testnet, defined in [BIP 13](https://github.com/litecoin/bips/blob/master/bip-0013.mediawiki)).
- Segwit addresses (bech32, of the form `bc1...` for mainnet or `tb1...` for testnet, defined in [BIP 173](https://github.com/litecoin/bips/blob/master/bip-0173.mediawiki)).
=======
- Hex encoded public keys (66 characters starting with `02` or `03`, or 130 characters starting with `04`).
  - Inside `wpkh` and `wsh`, only compressed public keys are permitted.
- [WIF](https://en.litecoin.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
-`xpub` encoded extended public key or `xprv` encoded private key (as defined in [BIP 32](https://github.com/litecoin/bips/blob/master/bip-0032.mediawiki)).
  - Followed by zero or more `/NUM` unhardened and `/NUM'` hardened BIP32 derivation steps.
  - Optionally followed by a single `/*` or `/*'` final step to denote all (direct) unhardened or hardened children.
  - The usage of hardened derivation steps requires providing the private key.
...
- P2SH addresses (base58, of the form `3...`, defined in [BIP 13](https://github.com/litecoin/bips/blob/master/bip-0013.mediawiki)).
- Segwit addresses (bech32, of the form `ltc1...`, defined in [BIP 173](https://github.com/litecoin/bips/blob/master/bip-0173.mediawiki)).
>>>>>>> 28c3cad38365b51883be89e7a306ac7eae1d9ba5
...
the entire path from the master down. [BIP174](https://github.com/litecoin/bips/blob/master/bip-0174.mediawiki) standardizes this by
...
All RPCs in Litecoin Core will include the checksum in their output. Only
