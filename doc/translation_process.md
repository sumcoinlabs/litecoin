...
Multiple language support is critical in assisting Litecoin’s global adoption, and growth. One of Litecoin’s greatest strengths is cross-border money transfers, any help making that easier is greatly appreciated.
...
The Transifex Litecoin project config file is included as part of the repo. It can be found at `.tx/config`, however you shouldn’t need to change anything.
...
3. Update `src/qt/litecoin_locale.qrc` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(litecoin_\(.*\)\).ts/<file alias="\2">locale\/\1.qm<\/file>/'
```
4. Update `src/Makefile.qt.include` manually or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename|sed 's/\(litecoin_\(.*\)\).ts/  qt\/locale\/\1.ts \\/'
```
5. Update `build_msvc/liblitecoin_qt/liblitecoin_qt.vcxproj` or via
```bash
git ls-files src/qt/locale/*ts|xargs -n1 basename |
  sed 's/@/%40/' |
  sed 's/\(litecoin_\(.*\)\).ts/    <None Include="..\\..\\src\\qt\\locale\\\1.ts">\n      <DeploymentContent>true<\/DeploymentContent>\n    <\/None>/'
```
...
To create a new language template, you will need to edit the languages manifest file `src/qt/litecoin_locale.qrc` and add a new entry. Below is an example of the English language entry.
