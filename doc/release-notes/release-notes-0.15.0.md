Litecoin Core version *0.15.0* is now available from:
  ...
  <https://litecoin.org/bin/litecoin-core-0.15.0/>
...
* When running Litecoin Core with multi-wallet, *wallet-level* RPC methods must specify the wallet for which they're intended in every request. HTTP RPC requests should be send to the `<RPC IP address>:<RPC port>/wallet/<wallet name>` endpoint, for example `127.0.0.1:8332/wallet/wallet1.dat`. `litecoin-cli` commands should be run with a `-rpcwallet` option, for example `litecoin-cli -rpcwallet=wallet1.dat getbalance`.
...
Litecoin Core has supported creating opt-in replace-by-fee (RBF) transactions
...
transaction with a higher-fee transaction are both supported in the GUI (See [PR 9592](https://github.com/litecoin/litecoin/pull/9592)).
...
- The second argument in `prioritisetransaction` has been renamed from `priority_delta` to `dummy` since Litecoin Core no longer has a concept of coin age priority. The `dummy` argument has no functional effect, but is retained for positional argument compatibility. See [Removal of Coin Age Priority](#removal-of-coin-age-priority).
...
- #10475 `39039b1` [RPC] getmempoolinfo mempoolminfee is a LTC/KB feerate (instagibbs)
...
- #10017 `02d64bd` combine_logs.py - aggregates log files from multiple litecoinds during functional tests (jnewbery)
...
- #10142 `ed09dd3` Run litecoin_test-qt under minimal QPA platform (ryanofsky)
...
- #10359 `329fc1d` [tests] functional tests should call LitecoinTestFramework start/stop node methods (jnewbery)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into LitecoinTestFramework (jnewbery)
...
- #10651 `cef4b5c` Verify binaries from litecoincore.org and litecoin.org (TheBlueMatt)
...
- #10958 `659c096` Update to latest Litecoin patches for LevelDB (sipa)
