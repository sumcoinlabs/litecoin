...
ClientModel is used by the main application `litecoingui` and several models like `peertablemodel`.
...
* UI elements like LitecoinAmountField, which inherit from QWidget.
* `litecoinstrings.cpp`: automatically generated
* `litecoinunits.(h/cpp)`: LTC / mLTC / etc handling
* `callback.h`
...
* don't change `local/litecoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
